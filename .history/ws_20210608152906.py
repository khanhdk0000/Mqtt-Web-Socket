...
def mqttGet(user, password,topic,device):
    ...
    client = mqtt.Client(client_id=str(random.randint(0,100))
...
def postlcd():
    ...
    mqttPost(TOPIC+LCD, USER, PASSWORD, '{ "id":"3", "name":"LCD", "data":"{}", "unit":"" }'.format(data))
    return 'yea: ' + data
...
def postbuzzer():
    ...
    data = input_json['data']
    print('receive data', data)
    mqttPost(TOPIC+BUZZER, USER, PASSWORD, '{ "id":"2", "name":"SPEAKER", "data":"{}", "unit":"" }'.format(data))
    return 'yea: ' + data
